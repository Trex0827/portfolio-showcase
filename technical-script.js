...
function renderSections(p){
  ...
  p.slides.forEach((raw,i)=>{
    ...
    renderSectionMedia(section.querySelector('.section-media'),p,i,0);
  });
}
...
function bindObservers(){
  sectionObservers.forEach(o=>o.disconnect()); sectionObservers=[];
  ...
  document.querySelectorAll('.tech-section').forEach(s=>observer.observe(s)); sectionObservers.push(observer);
}
